...
function KeysInner() {
  ...
  const createKey = async () => {
    ...
    try {
      ...
      toast(humaniseError(e, 'We could not create your API key. Please try again.'), 'error');
    }
    setCreating(false);
  };
  ...
  const act = async (key, action, extra = {}) => {
    ...
    try {
      ...
      toast(humaniseError(e, 'We could not update that API key. Please try again.'), 'error');
    }
    setBusyId(null);
  };
  ...
  const removeKey = async (key) => {
    ...
    try {
      ...
      toast(humaniseError(e, 'We could not delete that API key. Please try again.'), 'error');
    }
    setBusyId(null);
  }
  ...
  if (loading) return (
    <div className="container-site py-24 text-center">
      ...
      <p className="mono text-[11px] uppercase tracking-[0.14em]" style={{ color: 'var(--muted)' }}>Loading API keys…</p>
    </div>
  );
      ...
      <section className="section" style={{ paddingTop: 64, paddingBottom: 110 }}>
        <div className="container-site max-w-6xl">
          ...
          <div className="card card-pad mb-10">
            <div className="grid lg:grid-cols-12 gap-8 items-center">
              <div className="lg:col-span-4">
                <p className="mono text-[10px] uppercase tracking-[0.18em] mb-2" style={{ color: 'var(--brand)' }}>New key</p>
                <h2 className="section-title text-xl" style={{ color: 'var(--ink)' }}>Create an API key</h2>
          ...
          <div className="card overflow-hidden">
            <div className="px-6 py-4 flex items-center justify-between" style={{ borderBottom: '1px solid var(--line-soft)' }}>
              <h2 className="section-title text-xl" style={{ color: 'var(--ink)' }}>Your keys</h2>
              <span className="tag">{keys.length} total</span>
            </div>
            {keys.length === 0 ? (
              <div className="py-16 text-center">
                <KeyGlyph />
                <p className="display font-bold text-lg mt-4 mb-1" style={{ color: 'var(--ink)' }}>No API keys yet</p>
                <p className="text-sm" style={{ color: 'var(--muted)' }}>Create your first key above to start calling the API.</p>
              </div>
            ) : (
                  ...
                  <tbody>
                    {keys.map(k => (
                      <tr key={k.id} data-label="Name" style={{ opacity: k.status === 'revoked' ? 0.55 : 1 }}>
                        <td data-label="Name">
                          <p className="font-semibold" style={{ color: 'var(--ink)' }}>{k.name}</p>
                          <p className="mono text-[10px]" style={{ color: 'var(--dim)' }}>#{k.id}</p>
                        </td>
                        <td data-label="Key" className="mono text-[12px]" style={{ color: 'var(--ink-2)' }}>{k.prefix}</td>
                        ...
                        <td data-label="Requests">
                          <p className="mono text-[11px]" style={{ color: 'var(--ink-2)' }}>{k.total_requests.toLocaleString()} total</p>
                          <p className="mono text-[10px]" style={{ color: 'var(--dim)' }}>{k.requests_today.toLocaleString()} today</p>
                        </td>
                        <td data-label="Daily limit">
                          <p className="mono text-[11px]" style={{ color: 'var(--ink-2)' }}>{k.daily_limit < 0 ? '∞' : k.daily_limit.toLocaleString()}</p>
                          <p className="mono text-[10px]" style={{ color: k.remaining_today === 0 ? 'var(--bad)' : 'var(--dim)' }}>
                            {k.remaining_today < 0 ? 'unlimited' : `${k.remaining_today.toLocaleString()} left today`}
                          </p>
                        </td>
                        <td data-label="Last used" style={{ color: 'var(--ink-2)' }}>{fmtDate(k.last_used_at)}</td>
                        <td data-label="Created" style={{ color: 'var(--muted)' }}>{fmtDate(k.created_at)}</td>
                        <td data-label="Expires" style={{ color: k.expires_at ? 'var(--brand)' : 'var(--muted)' }}>{fmtDate(k.expires_at)}</td>
                        <td data-label="Actions">
                          <div className="flex flex-wrap gap-1.5">
                            <button onClick={() => act(k, 'rename', { name: prompt('New key name:', k.name) || k.name })}
                              ...
                              style={{ border: '1px solid var(--line)', color: 'var(--ink-2)', background: 'transparent', cursor: 'pointer' }}>
                              Rename
                            </button>
                            {k.status === 'active' ? (
                              <>
          ...
          {newKey && (
            <div className="fixed inset-0 z-[90] flex items-center justify-center p-4"
              ...
              <div className="card card-pad w-full max-w-lg" style={{ background: 'var(--bg-2)' }} onClick={e => e.stopPropagation()}>
                <div className="flex items-center gap-3 mb-2">
                  <KeyGlyph />
                  <div>
                    <h3 className="display font-bold text-lg" style={{ color: 'var(--ink)' }}>Your new API key</h3>
                    <p className="mono text-[10px] uppercase tracking-[0.14em] mt-0.5" style={{ color: 'var(--brand)' }}>
