...
// Manual login: verifies the bcrypt password hash stored in the Neon `users`
// table and mints the site JWT session cookie.
  ...
  try {
    ...
    if (!user.password) {
      return NextResponse.json(
        { error: 'No password set on this account. Please use "Forgot password" to set one.' },
        { status: 401 }
      );
    }
