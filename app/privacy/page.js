...
const SECTIONS = [
  { id: 'collect', title: '1. What We Collect', body: (
    <>
      <p><strong style={{ color: 'var(--ink)' }}>Account data:</strong> name, email, and password (hashed).{' '}
      <strong style={{ color: 'var(--ink)' }}>Payment data:</strong> handled by Paystack — we never store card details.{' '}
      <strong style={{ color: 'var(--ink)' }}>Usage data:</strong> API requests, panel usage, and wallet transactions for billing and analytics.{' '}
      <strong style={{ color: 'var(--ink)' }}>WhatsApp bot data:</strong> session tokens only — never your messages or contacts.</p>
    </>
  ) },
