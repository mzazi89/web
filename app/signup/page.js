...
export default function SignupPage() {
  ...
  // Mint the site session cookie + save signup extras (security question, referral)
  const syncSession = async (extras = {}) => {
    try {
  ...
  const finishSignup = async (extras) => {
    await setSignUpActive({ session: signUp.createdSessionId });
    await syncSession(extras);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.status === 'complete') {
        await finishSignup(extras);
      } else if (result.status === 'missing_requirements') {
        // Email verification required (per Clerk dashboard settings)
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      setError('Google Sign-In failed. Please try again or register with email.');
    }
  };
        ...
        <div className="rounded-2xl p-8" style={{ backgroundColor: '#16182a', border: '1px solid #1e2d4a', boxShadow: '0 25px 50px rgba(0,0,0,0.5)' }}>
          <div className="text-center mb-8">
            ...
            <p className="text-sm mt-1" style={{ color: '#64748b' }}>
              {step === 'form' ? "Join MZAZI TECH — it's free" : `We sent a 6-digit code to ${formData.email}`}
            </p>
          ...
          {step === 'verify' ? (
            <form onSubmit={handleVerify} className="space-y-5">
              <div>
                <label className="block text-sm font-medium mb-2" style={{ color: '#94a3b8' }}>Verification Code</label>
                <input
                  type="text"
                  required
                  value={code}
                  ...
                  className="w-full rounded-xl px-4 py-3 text-sm outline-none text-center tracking-[0.4em]"
                  style={inputStyle}
                  onFocus={e => e.target.style.borderColor = '#2563eb'}
                  onBlur={e => e.target.style.borderColor = '#1e2d4a'}
                />
                <p className="text-[11px] mt-2" style={{ color: '#475569' }}>
                  Check your inbox — the code expires shortly.
                </p>
              </div>
              ...
              <button type="button" onClick={() => setStep('form')} className="w-full py-2 text-xs"
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="mb-6 p-4 rounded-xl" style={{ backgroundColor: 'rgba(37,99,235,0.05)', border: '1px solid rgba(37,99,235,0.2)' }}>
                  ...
                  <input type="text" name="securityAnswer" required value={formData.securityAnswer} onChange={handleChange}
                    ...
                    onFocus={e => e.target.style.borderColor = '#2563eb'} onBlur={e => e.target.style.borderColor = '#1e2d4a'} />
                  <p className="text-[11px] mt-2" style={{ color: '#475569' }}>
                    Password recovery now uses email codes — this question is kept as an extra account security step.
                  </p>
                </div>

                <button type="submit" disabled={loading || !signUpLoaded} className="w-full py-3 rounded-xl font-bold text-white text-sm transition-all"
                  style={{ background: loading ? '#1e2d4a' : 'linear-gradient(135deg, #2563eb, #1d4ed8)', cursor: loading ? 'not-allowed' : 'pointer' }}>
                  {loading ? (
