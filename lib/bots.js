...
export async function resolveBotForNumber(number, requested) {
  ...
  // With a single bot there is nothing to disambiguate, so the row is left
  // untargeted. Stamping it with the fallback id "main" would look equivalent
  // today and then silently strand the request if bot_profiles is switched on
  // before the bot claims it — a targeted row naming a profile that no longer
  // exists. Untargeted is claimable by whatever is running.
