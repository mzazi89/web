...
// prices are in KSH (the wallet stores KSH; the UI shows MTC via fmtMtc)
export const PLANS = {
...
export async function ensureAccount(sql, siteUserId) {
  const rows = await sql`
    ...
    VALUES (${Number(siteUserId)})
    ON CONFLICT ("telegramId") DO UPDATE SET "telegramId" = EXCLUDED."telegramId"
    RETURNING id
  `;
  return rows[0];
}
