...
export async function initializeDatabase() {
  try {
    ...
    await sql`
      INSERT INTO providers (name, base_url, api_key_configured, status)
      VALUES ('davidcyril', ${process.env.DAVIDCYRIL_API_URL || 'https://apis.davidcyril.name.ng'},
    ...
    // Seed endpoint registry from the DavidCyril inventory (444 endpoints cataloged;
    ...
    const REGISTRY_PATH = path.join(process.cwd(), 'lib', 'api', 'registry', 'davidcyril.json');
    if (fs.existsSync(REGISTRY_PATH)) {
      ...
      for (const ep of registry) {
        ...
        await sql`
          ...
          VALUES (${ep.path}, ${ep.method}, ${ep.category}, ${ep.name}, ${ep.name}, 'davidcyril',
      ...
      console.log(`Seeded ${registry.length} endpoints from registry`);
    }
