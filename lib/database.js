...
export async function initializeDatabase() {
  try {
    ...
    // ── Multi-payment deposit columns (safe on re-run) ──────────────────────
    // Status values: pending → processing → success | failed | abandoned
    await sql`ALTER TABLE wallet_transactions ADD COLUMN IF NOT EXISTS currency VARCHAR(10) DEFAULT 'KES'`;
    await sql`ALTER TABLE wallet_transactions ADD COLUMN IF NOT EXISTS payment_method VARCHAR(50) DEFAULT 'card'`;
    ...
    await sql`ALTER TABLE wallet_transactions ADD COLUMN IF NOT EXISTS paystack_transaction_id BIGINT`;
    ...
    // Unique reference — the idempotency anchor for webhook/verify processing.
    // Dedupe any legacy duplicates first (keep the newest row per reference),
    // then enforce uniqueness so a reference can only ever be claimed once.
    try {
      await sql`
      ...
      console.warn('init: wallet_transactions reference index skipped —', e.message);
    }
    await sql`CREATE INDEX IF NOT EXISTS idx_wallet_tx_user ON wallet_transactions(user_id, created_at DESC)`;
