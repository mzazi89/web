...
export async function initializeDatabase() {
  try {
    ...
    // Legacy column migrations — non-fatal: if the table doesn't exist yet
    // (fresh database), skip and let the CREATE TABLE below handle it.
    try {
      await sql`ALTER TABLE packages ADD COLUMN IF NOT EXISTS expires_after_hours INTEGER DEFAULT NULL`;
    } catch (e) {
      console.warn('init: packages ALTER skipped —', e.message);
    }
    try {
      await sql`ALTER TABLE panels ADD COLUMN IF NOT EXISTS expires_at TIMESTAMP DEFAULT NULL`;
    } catch (e) {
      console.warn('init: panels ALTER skipped —', e.message);
    }
    ...
    // Multi-method deposit support (additive — safe on re-run).
    // currency/payment_method/phone_number/provider/paystack_id/paid_at track
    // Paystack Charge API deposits (M-PESA, Airtel Money, M-PESA Till, Card).
    ...
    // Ludo multiplayer games (server-authoritative JSONB state)
    await sql`
    ...
    // multi-provider failover chains: [{ provider, upstream }, ...] tried in order
    await sql`ALTER TABLE endpoints ADD COLUMN IF NOT EXISTS fallbacks JSONB DEFAULT '[]'::jsonb`;
    ...
    await sql`
      INSERT INTO providers (name, display_name, base_url, api_key_configured, status)
      VALUES ('drexapp', 'Mzazi Tech Inc', ${process.env.DREXAPP_API_URL || 'https://apis-17ad50309099.herokuapp.com'},
    ...
    // Keyless providers — no API keys required (added to keep the platform
    // healthy when the keyed upstreams are unavailable)
    await sql`
      INSERT INTO providers (name, display_name, base_url, api_key_configured, status)
      VALUES ('pollinations', 'Pollinations AI', ${process.env.POLLINATIONS_API_URL || 'https://text.pollinations.ai'},
              false, 'active')
      ON CONFLICT (name) DO UPDATE SET
    ...
    await sql`
      INSERT INTO providers (name, display_name, base_url, api_key_configured, status)
      VALUES ('keyless', 'Keyless Free APIs', 'https://', false, 'active')
      ON CONFLICT (name) DO UPDATE SET
        display_name = 'Keyless Free APIs',
        api_key_configured = false
    `;
    await sql`
      INSERT INTO providers (name, display_name, base_url, api_key_configured, status)
      VALUES ('ytdl', 'YouTube Multi-Instance', 'https://', false, 'active')
      ON CONFLICT (name) DO UPDATE SET
        display_name = 'YouTube Multi-Instance',
        api_key_configured = false
    `;
    await sql`
      INSERT INTO providers (name, display_name, base_url, api_key_configured, status)
      VALUES ('social', 'Social Media (FixVX)', 'https://', false, 'active')
      ON CONFLICT (name) DO UPDATE SET
        display_name = 'Social Media (FixVX)',
        api_key_configured = false
    `;
    // Your own API — base URL + key come from env (MYAPI_*)
    await sql`
      INSERT INTO providers (name, display_name, base_url, api_key_configured, status)
      VALUES ('myapi', 'My Own API', ${process.env.MYAPI_API_URL || ''}, ${Boolean(process.env.MYAPI_API_KEY)}, 'active')
      ON CONFLICT (name) DO UPDATE SET
        display_name = 'My Own API',
        base_url = EXCLUDED.base_url,
        api_key_configured = EXCLUDED.api_key_configured
    `;
    ...
    const REGISTRIES = [
      ...
      { file: 'pollinations.json', provider: 'pollinations' },
      { file: 'keyless.json', provider: 'keyless' },
      { file: 'ytdl.json', provider: 'ytdl' },
      { file: 'social.json', provider: 'social' },
      { file: 'myapi.json', provider: 'myapi' },
    ];
    // Deterministic precedence for shared paths. During a deploy, many
    // serverless instances seed concurrently; the last writer would win for
    // paths present in BOTH a legacy registry (davidcyril/drexapp) and an
    // override registry (keyless/ytdl/social/myapi/pollinations), which could
    // resurrect a dead provider (e.g. /api/download/play → davidcyril).
    // Override registries therefore own their paths exclusively — enforced
    // both here (legacy registries skip override paths) and in the upsert's
    // WHERE clause (legacy rows may only be replaced by override providers).
    const legacyProviders = ['davidcyril', 'drexapp'];
    const overrideProviders = REGISTRIES.filter(r => !legacyProviders.includes(r.provider)).map(r => r.provider);
    const overridePaths = new Set();
    for (const { file, provider } of REGISTRIES) {
      if (legacyProviders.includes(provider)) continue;
      const overridePath = path.join(REGISTRY_DIR, file);
      if (!fs.existsSync(overridePath)) continue;
      for (const ep of JSON.parse(fs.readFileSync(overridePath, 'utf8'))) {
        overridePaths.add(ep.path);
      }
    }
    ...
    for (const { file, provider } of REGISTRIES) {
      ...
      for (const ep of registry) {
        // Legacy registries must not overwrite override-owned paths
        if (legacyProviders.includes(provider) && overridePaths.has(ep.path)) continue;
        const paramsDef = {
        ...
        await sql`
          INSERT INTO endpoints (path, method, category, name, description, provider, upstream, parameters, requires_query, is_active, fallbacks)
          ...
          WHERE endpoints.provider = ANY(${legacyProviders})
             OR EXCLUDED.provider = ANY(${overrideProviders})
        `;
      }
    ...
    `;
    // Which bot a command belongs to. The bots add this themselves, but the
    // website now serves per-bot registries (GET /api/xmd-command filters on
    // it), so the website must be able to guarantee it exists rather than
    // depending on a bot having run first. '' means the row predates bot
    // profiles and belongs to the primary bot, so nothing disappears here.
    await sql`ALTER TABLE bot_commands ADD COLUMN IF NOT EXISTS profile VARCHAR(64) DEFAULT ''`;
    ...
    await sql`CREATE INDEX IF NOT EXISTS idx_bot_commands_profile ON bot_commands(profile)`;

    // Per-profile uniqueness. `name` was UNIQUE globally, so the two bots could
    // not both own a command called `menu` — the second bot's INSERT failed on
    // the other bot's row. Uniqueness is now scoped to (profile, name): the
    // same command name may exist once per bot, but never twice for one bot.
    // Both statements are idempotent and additive, so this is safe to run
    // against the live database on every boot.
    await sql`ALTER TABLE bot_commands DROP CONSTRAINT IF EXISTS bot_commands_name_key`;
    await sql`CREATE UNIQUE INDEX IF NOT EXISTS bot_commands_profile_name_key ON bot_commands(profile, name)`;

    // bot_control.bot_id — the bot a control row is addressed to. The bot adds
    // this to itself on boot, but the admin can enqueue a targeted control (and
    // must be able to address one bot when two are running) before any bot has
    // ever run against this database, so the column has to exist here too.
    await sql`ALTER TABLE bot_control ADD COLUMN IF NOT EXISTS bot_id VARCHAR(64) DEFAULT ''`;
    await sql`CREATE INDEX IF NOT EXISTS idx_bot_control_bot_id ON bot_control(bot_id)`;
    ...
    // bot_config — key/value settings for the bot platform (e.g. bot_api_key)
    await sql`
...
// Self-healing: if bot_commands disappears after init (e.g. an external
// "prisma db push" dropped it), the next call re-runs the full init.
...
export async function ensureDatabase() {
  ...
  try {
    await sql`SELECT 1 FROM bot_commands LIMIT 1`;
  } catch {
    _dbInitPromise = null;
    _dbInitPromise = initializeDatabase().catch((e) => {
      _dbInitPromise = null;
