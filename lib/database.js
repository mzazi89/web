...
export async function initializeDatabase() {
  try {
    ...
    // Override registries therefore own their paths exclusively.
    const overridePaths = new Set();
    for (const { file, provider } of REGISTRIES) {
      if (provider === 'davidcyril' || provider === 'drexapp') continue;
    ...
    for (const { file, provider } of REGISTRIES) {
      ...
      for (const ep of registry) {
        ...
        if ((provider === 'davidcyril' || provider === 'drexapp') && overridePaths.has(ep.path)) continue;
        const paramsDef = {
