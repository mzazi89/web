...
export async function getFulfilledVps(reference) {
  const rows = await sql`
    SELECT o.id AS order_id, o.user_id, o.amount, o.payment_method, o.reference, o.status,
           ...
           p.id AS package_id, p.name AS package_name, p.ram, p.cpu, p.disk, p.bandwidth,
           p.location, p.os, p.description, p.price
    FROM vps_orders o
...
export async function fulfillVpsOrderIfPaid(reference) {
  ...
  const rows = await sql`
    WITH claim AS (
      ...
      RETURNING i.id, i.host, i.username, i.password, i.port
    ),
    ord AS (
      UPDATE vps_orders
      SET status = 'success',
    ...
    SELECT o.user_id, o.package_id, o.amount,
           ...
           p.name AS package_name, p.ram, p.cpu, p.disk, p.bandwidth,
           p.location, p.os, p.description, p.price
    FROM ord o
  ...
  const vps = {
    ...
    ram: r.ram, cpu: r.cpu, disk: r.disk, bandwidth: r.bandwidth,
    location: r.location, os: r.os, description: r.description,
