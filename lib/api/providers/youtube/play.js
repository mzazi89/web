...
//   YOUTUBE_API_URL  — e.g. https://apis.davidcyril.name.ng/play (required)
//   YOUTUBE_API_KEY  — optional provider credential. Only appended to the
//                      upstream request when actually set. Never send
//                      apikey=undefined or apikey= empty.
...
// Only the base URL is required — the provider API key is optional
...
function buildUpstreamUrl(query) {
  ...
  // Only append the provider key when it actually exists
  if (UPSTREAM_KEY) {
...
export function normalizeResponse(upstream) {
  ...
  // download_url often nested: { download: {...} } or flat (snake/camelCase)
  const dl = pick(r, ['download', 'audio', 'mp3']);
  const download_url = toHttps(
    pick(r, ['download_url', 'downloadUrl', 'audio_url', 'mp3_url', 'direct_url']) ||
    (typeof dl === 'object' ? pick(dl, ['url', 'download_url', 'downloadUrl', 'audio', 'link']) : null)
  );

  const video_url = toHttps(
    pick(r, ['video_url', 'videoUrl', 'youtube_url', 'watch_url', 'link', 'source_url', 'url']) ||
    (typeof r.video === 'object' ? pick(r.video, ['url', 'video_url', 'videoUrl']) : null)
  );
